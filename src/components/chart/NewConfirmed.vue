   ...
   watch: {
      data: async function () {
        ...
        await this.data.then((response) => {
            let aux = 0; 
            for(let i = 0; i < 30; i++) {
               if(response.data.results[aux].new_confirmed == 0 && response.data.results[aux+1].last_available_date == response.data.results[aux].last_available_date) {
                  do {
                     aux++
                  } while(response.data.results[aux].new_confirmed == 0 && response.data.results[aux+1].last_available_date == response.data.results[aux].last_available_date)
               }
